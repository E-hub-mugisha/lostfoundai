...
Route::get('/', function () {
    $lostIdCount = \App\Models\ExtractedDocument::where('status', 'lost')->count();
    $foundIdCount = \App\Models\ExtractedDocument::where('status', 'found')->count();
    $userCount = \App\Models\User::count();
    return view('home' , compact('lostIdCount', 'foundIdCount', 'userCount'));
});
...
// Lost and Found Documents routes (authenticated users)
Route::middleware('auth')->group(function () {
    ...
    Route::get('/lost-documents/{lost_document}/edit', [LostDocumentController::class, 'edit'])->name('lost-documents.edit');
...
// Admin & Manager routes (for reports)
Route::middleware(['auth', RoleMiddleware::class . ':admin'])->group(function () {
